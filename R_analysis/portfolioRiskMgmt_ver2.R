...
# download data from Yahoo! Finance Japan 
s1306 <- quoteStockTsData('1306.t', since = '2008-01-01', date.end = NULL, time.interval = 'daily')
s1681 <- quoteStockTsData('1681.t', since = '2008-01-01', date.end = NULL, time.interval = 'daily')
s1680 <- quoteStockTsData('1680.t', since = '2008-01-01', date.end = NULL, time.interval = 'daily')
s8282 <- quoteStockTsData('8282.t', since = '2008-01-01', date.end = NULL, time.interval = 'daily')
s3092 <- quoteStockTsData('3092.t', since = '2008-01-01', date.end = NULL, time.interval = 'daily')

stock1306 <- timeSeries(data=s1306[,"adj_close"], charvec=s1306[,"date"],zone="Asia/Tokyo",FinCenter="Asia/Tokyo",units="close")
stock1681 <- timeSeries(data=s1681[,"adj_close"], charvec=s1681[,"date"],zone="Asia/Tokyo",FinCenter="Asia/Tokyo",units="close")
stock1680 <- timeSeries(data=s1680[,"adj_close"], charvec=s1680[,"date"],zone="Asia/Tokyo",FinCenter="Asia/Tokyo",units="close")
stock8282 <- timeSeries(data=s8282[,"adj_close"], charvec=s8282[,"date"],zone="Asia/Tokyo",FinCenter="Asia/Tokyo",units="close")
stock3092 <- timeSeries(data=s3092[,"adj_close"], charvec=s3092[,"date"],zone="Asia/Tokyo",FinCenter="Asia/Tokyo",units="close")
...
holdings <- c(140,410,330,100,100)

# 2010-02-24以降にMSCI-EMERGING上場なので、それ以降のデータしかない
# スタートトゥデイは2011/1/27に株式分割１株->300.03株したので、２６日以前の株価を調整する。!! Yahooからデータを取った場合は不要の処理
...
sqrt(rollVar(ptf.ret[,1], n=100, unbiased=FALSE))[100]
sqrt(rollVar(ptf.ret[,2], n=100, unbiased=FALSE))[100]
sqrt(rollVar(ptf.ret[,3], n=100, unbiased=FALSE))[100]
sqrt(rollVar(ptf.ret[,4], n=100, unbiased=FALSE))[100]
sqrt(rollVar(ptf.ret[,5], n=100, unbiased=FALSE))[100]
sqrt(rollVar(ptf.ret[,6], n=100, unbiased=FALSE))[100]

# 確かにポートフォリオのボラティリティが0.0078と、世界株の0.0074についで低い。
# 個別株のボラティリティよりは、遥かに低い

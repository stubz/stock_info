...
def insertReturnAndVolatEstimateToTable(con, stockcd, mktcd, date, ret, logret, ret3mon, volat3mon,
    ...
    sql = """
        ...
        """
    sql = sql % (stock_db.tblPerfMonthly, date, stockcd, mktcd, ret, logret, ret3mon, volat3mon,
            ret6mon, volat6mon, ret1year, volat1year, ret3year, volat3year, ret5year, volat5year)
    # print sql
...
def calcMonthlyReturnAndVolatForStockData(stockData, latestdate):
    ...
    for stockcd, mktcd in stockData.keys():
        ...
        for i in range(len(ret)):
            ...
            # stockPerfMonthlyの最新日付以降についてのみ計算を行う。データは日付の新しい順に入っている前提
            if len(ret[i:]) < 3 or datelist[i] <= latestdate:
                break
            ...
            retvol_matrix = stock_db.getReturnAndVolatEstimateByPeriod(ret[i:], data_length=[
                3, 6, stock_db.MONTH_YEAR, stock_db.MONTH_YEAR*3, stock_db.MONTH_YEAR*5])
...
if __name__=='__main__':
    ...
    print("\n\n%s\n\n" % ("*"*80))
    print("******* calculate estimated returns and volatilities from %s *******\n\n" % (stock_db.tblPriceMonthly))

    con = stock_db.connect()

    # delete all rows in stockPerfMonthly
    # sql = "truncate table stockPerfMonthly;"
    # cur.execute(sql)

    print " getting latest date of the stockPerfMonthly ",
    latestdate = stock_db.getLastDateFromTable(stock_db.tblPerfMonthly)
    # 初めて計算するときはstockPerfMonthlyが空なので、stockPriceHistMonthlyの始めの日付にする
    # latestdate = datetime.date(2000,1,4)
    # 日時データが存在する最新の日付を取得する
    enddate   = stock_db.getLastDateFromTable(stock_db.tblPriceMonthly)
    # 最新の日付を取得する
    ...
    for mktcd in mktcdlist:
        print "extracting data from %s between %s and %s" % (stock_db.tblPriceMonthly, startdate, latestdate)
        stockData = stock_db.getStockHistDailyByDate2(start=startdate, end=enddate, mktcd=mktcd, tblStockPriceHist=stock_db.tblPriceMonthly, columns=['close'])
        print "\n----- start computing return and volatility for market code = '%s'-----\n" % ( mktcd )
        calcMonthlyReturnAndVolatForStockData(stockData, latestdate)

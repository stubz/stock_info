...
if __name__=='__main__':
    ...
    latestdate = stock_db.getLastDateInStockChangeDaily()

    # 変化率テーブルの最新日付を含む日付リストを取得
    datelist = stock_db.getDailyDateListAfter(latestdate, True)
    ...
    for i in range(len(datelist)-1):
        ...
        # stockdata = stock_db.getStockDataAllByDate(start=datelist[i], end=datelist[i+1], columns=['close'], tblStockPriceHist='stockPriceHistDaily')
        stockdata = stock_db.getStockDataAllByDate(start=datelist[i], end=datelist[i+1], columns=['close'], tblStockPriceHist=stock_db.tblPriceDaily)
        cnt = 0
        ...
        # for code, mktcd in cdlist:
            ...
            sql = """INSERT INTO %s (date, stockcd, marketcd, ret, logret) 
                values('%s', '%s', '%s', %f, %f)""" % (stock_db.tblPriceChangeDaily, datelist[i+1], code, mktcd, ret, logret)
            try:
                cur.execute(sql)
            except:

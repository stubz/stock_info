...
def insertReturnAndVolatEstimateToTable(con, stockcd, mktcd, date, ret, logret, ret3mon, volat3mon,
    ...
    sql = """
        ...
        """
    sql = sql % (stock_db.tblPerfWeekly, date, stockcd, mktcd, ret, logret, ret3mon, volat3mon,
            ret6mon, volat6mon, ret1year, volat1year, ret3year, volat3year, ret5year, volat5year)
    # print sql
...
def calcWeeklyReturnAndVolatForStockData(stockData, latestdate):
    ...
    for stockcd, mktcd in stockData.keys():
        ...
        for i in range(len(ret)):
            ...
            # stockPerfWeeklyの最新日付以降についてのみ計算を行う。データは日付の新しい順に入っている前提
            if len(ret[i:]) < stock_db.WEEK_MONTH*3 or datelist[i] <= latestdate:
                break
            ...
            retvol_matrix = stock_db.getReturnAndVolatEstimateByPeriod(ret[i:], data_length=[
                stock_db.WEEK_MONTH*3, stock_db.WEEK_MONTH*6, stock_db.WEEK_YEAR, stock_db.WEEK_YEAR*3, stock_db.WEEK_YEAR*5])
...
if __name__=='__main__':
    ...
    print("\n\n%s\n\n" % ("*"*80))
    print("******* calculate estimated returns and volatilities from %s *******\n\n" % (stock_db.tblPriceWeekly))

    con = stock_db.connect()

    # delete all rows in stockPerfWeekly
    # sql = "truncate table stockPerfWeekly;"
    # cur.execute(sql)

    print " getting latest date of the stockPerfWeekly ",
    latestdate = stock_db.getLastDateFromTable(stock_db.tblPerfWeekly)
    # 初めて計算するときはstockPerfWeeklyが空なので、stockPriceHistWeeklyの始めの日付にする
    # latestdate = datetime.date(2000,1,4)
    # 日時データが存在する最新の日付を取得する
    enddate   = stock_db.getLastDateFromTable(stock_db.tblPriceWeekly)
    # 最新の日付を取得する
    # startdate = datetime.date(2000, 1, 4)
    ...
    for mktcd in mktcdlist:
        print "extracting data from %s between %s and %s" % (stock_db.tblPriceWeekly, startdate, latestdate)
        stockData = stock_db.getStockHistDailyByDate2(start=startdate, end=enddate, mktcd=mktcd, tblStockPriceHist=stock_db.tblPriceWeekly, columns=['close'])
        print "\n----- start computing return and volatility for market code = '%s'-----\n" % ( mktcd )
        calcWeeklyReturnAndVolatForStockData(stockData, latestdate)

...
if __name__=='__main__':
    ...
    # 最新の銘柄コードと市場コードのタプルリストを取得する。
    # 日次データテーブルから取得する
    cdlist = stock_db.getStockCodeListCurrent()
    # get a list of dates
    # 変化率テーブルの最新日付を取得
    # １回目（初めてテーブルにデータを入れる時）は日付を指定して実行
    latestdate = datetime.date(2001, 1, 31)
    # latestdate = stock_db.getLastDateInStockChangeMonthly()

    # 変化率テーブルの最新日付を含む日付リストを取得
    datelist = stock_db.getMonthlyDateListAfter(latestdate, True)
    ...
    for i in range(len(datelist)-1):
        ...
        stockdata = stock_db.getStockDataAllByDate(start=datelist[i], end=datelist[i+1], columns=['close'], tblStockPriceHist=stock_db.tblPriceMonthly)
        cnt = 0
        ...
        # for code, mktcd in cdlist:
            ...
            sql = """INSERT INTO %s (date, stockcd, marketcd, ret, logret) 
                values('%s', '%s', '%s', %f, %f)""" % (stock_db.tblPriceChangeMonthly, datelist[i+1], code, mktcd, ret, logret)
            try:
                cur.execute(sql)
            except:
